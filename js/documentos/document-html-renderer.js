const DocumentHtmlRenderer = {
    ...
    renderizarClienteProjeto(dados = {}) {
        ...
        const servicos = Array.isArray(dados.servicos) && dados.servicos.length
            ...
            : servico.nome;
        const projetoNome = projeto.nome || projeto.numero || projeto.id;
        const tituloServico = projetoNome ? "Projeto / servi\u00e7o" : "Servi\u00e7o";
        ...
        return [
            ...
            this.renderizarSecao(tituloServico, [
                this.renderizarCampo("Projeto", projetoNome),
    ...
    renderizarProdutos(dados = {}) {
        ...
        const linhas = produtos.map(produto => [
            ...
            `<td>${this.escapar(this.rotuloItemPdf(produto.tipoItemNome || produto.nome))}</td>`,
            `<td>${this.escapar(this.rotuloItemPdf(produto.subtipoItem))}</td>`,
            `<td>${this.escapar(this.rotuloItemPdf(produto.descricao || produto.nome) || "Item")}</td>`,
            `<td>${this.escapar(this.formatarNumero(produto.alturaCm || produto.altura))}</td>`,
            `<td>${this.escapar(this.formatarNumero(produto.larguraCm || produto.largura))}</td>`,
        ...
        return [
            ...
            `<table>`,
            `<thead><tr><th>Item</th><th>Tipo</th><th>Subtipo</th><th>Descri&ccedil;&atilde;o</th><th>Altura (cm)</th><th>Largura (cm)</th><th>Qtd.</th><th>Valor (m&sup2;)</th><th>Subtotal</th></tr></thead>`,
            `<tbody>${linhas}</tbody>`,
            `</table>`,
            `</div>`,
            `</section>`
    ...
    renderizarTotais(dados = {}) {
        ...
        const produtos = Array.isArray(dados.produtos) ? dados.produtos : [];
        const totalAdicionais = Number(totais.totalAdicionais || 0) > 0
        ...
        const totalAluminio = Number(totais.totalAluminio || 0) > 0
            ...
            : produtos
                .reduce((total, produto) => total + Math.max(0, Number(produto.totalAluminio || 0)), 0);
        const totalJato = Number(totais.totalJato || 0) > 0
            ? Number(totais.totalJato)
            : produtos.reduce(
                (total, produto) => total + Math.max(0, Number(produto.valorJato || produto.adicionalJato || 0)),
                0
            );

        const campos = [
            this.renderizarCampo("Subtotal", this.formatarMoeda(totais.subtotal, moeda), true),
            totalAdicionais > 0 ? this.renderizarCampo("Ferragens/Acessórios", this.formatarMoeda(totalAdicionais, moeda), true) : "",
            totalAluminio > 0 ? this.renderizarCampo("Aluminio", this.formatarMoeda(totalAluminio, moeda), true) : "",
            totalJato > 0 ? this.renderizarCampo("Jato", this.formatarMoeda(totalJato, moeda), true) : "",
