const NotaServicoUI = {
    ...
    renderizarHistorico(itens) {
        ...
        lista.innerHTML = itens.map(item => `<article><div><span>${this.escapar(item.numeroNota)}</span><strong>${this.escapar(item.clienteNome)}</strong><small>${this.dataBr(item.dataEmissao)} • ${item.servicos.length} ${item.servicos.length === 1 ? "serviço" : "serviços"}</small></div><b>${NotaServicoModel.moeda(NotaServicoModel.total(item))}</b><div class="nota-historico-acoes"><button type="button" data-historico="abrir" data-id="${this.escapar(item.id)}">Abrir</button><button type="button" data-historico="pdf" data-id="${this.escapar(item.id)}">PDF</button><button class="perigo" type="button" data-historico="excluir" data-id="${this.escapar(item.id)}">Excluir</button></div></article>`).join("");
    },
