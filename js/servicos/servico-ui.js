const ServicoUI = {
            ...
            form.addEventListener("reset", () => {
                window.setTimeout(() => {
                    ...
                    if (this.elementos.formTitulo) {
                        this.elementos.formTitulo.textContent = "Cadastro de item";
                    }
                }, 0);
            });
        }
    ...
    preencherFormulario(servico = {}) {
        ...
        if (this.elementos.formTitulo) {
            this.elementos.formTitulo.textContent = servico.id ? "Editar item" : "Cadastro de item";
        }
    ...
    renderizarLista(servicos = []) {
        ...
        if (!servicos.length) {
            tabelaCorpo.innerHTML = `
                <tr>
                    <td colspan="6" class="cadastro-estado-vazio">Nenhum item cadastrado.</td>
                </tr>
            `;
            return;
        }
        ...
        tabelaCorpo.innerHTML = servicos.map(servico => `
            <tr>
                <td>
                    <strong>${this.escapar(servico.nome || "Item")}</strong>
    ...
    renderizarDetalhe(servico) {
        ...
        if (!servico) {
            if (detalheResumo) detalheResumo.textContent = "Selecione um item para visualizar.";
            if (detalheConteudo) detalheConteudo.innerHTML = `<p class="cadastro-placeholder">Nenhum item selecionado.</p>`;
            return;
        }

        if (detalheResumo) {
            detalheResumo.textContent = `${servico.nome || "Item"} pronto para o catalogo comercial.`;
        }

        if (detalheConteudo) {
            detalheConteudo.innerHTML = `
                <div class="cadastro-resumo-grid">
                    ${this.renderizarCampo("Grupo", this.rotuloCategoria(servico.categoria))}
    ...
    renderizarTipos() {
        ...
        if (!this.tiposAtuais.length) {
            lista.innerHTML = `<p class="cadastro-placeholder">Nenhum tipo de item.</p>`;
            return;
        }
    ...
    adicionarTamanho() {
        ...
        this.tamanhosAtuais.push({
            ...
            tipoItem,
            nome: nome || `${alturaCm} x ${larguraCm} cm`,
            larguraCm,
            alturaCm,
            ativo: true,
        ...
        lista.innerHTML = this.tamanhosAtuais.map((tamanho, indice) => `
            <div class="cadastro-lista-item">
                <div>
                    ...
                    <small>${this.escapar(`${tamanho.tipoItem || "Modelo"} - ${tamanho.alturaCm || 0} x ${tamanho.larguraCm || 0} cm - ${this.calcularAreaM2(tamanho.larguraCm, tamanho.alturaCm)} m2`)}</small>
                </div>
        ...
        return `
            <section class="cadastro-detalhe-bloco">
                ...
                ${itens.length
                    ...
                    `).join("")
                    : `<p class="cadastro-placeholder">Nenhum tipo de item.</p>`}
            </section>
        `;
    },
    ...
    atualizarResumo(total) {
        ...
        this.elementos.resumo.textContent = total === 1 ? "1 item cadastrado" : `${total} itens cadastrados`;
    },
