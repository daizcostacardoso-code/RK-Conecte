const PdfAdapter = {
    ...
    async gerarArquivoPdf(exportacao = {}) {
        try {
            ...
            if (this.projetoPreenchido(dados.projeto)) {
    ...
    criarEstadoPdf(pdfLib, pdfDoc, fonte, fonteNegrito) {
        ...
        return {
            ...
            margem,
            y: altura - margem,
            corTexto: pdfLib.rgb(0.02, 0.16, 0.32),
            corSuave: pdfLib.rgb(0.20, 0.26, 0.32),
            corPrimaria: pdfLib.rgb(0.07, 0.42, 1),
            corLinha: pdfLib.rgb(0.70, 0.84, 0.98),
            corFundo: pdfLib.rgb(0.90, 0.96, 1)
        };
    },

    novaPagina(estado) {
        estado.pagina = estado.pdfDoc.addPage([595.28, 841.89]);
    ...
    desenharCabecalho(estado, dados = {}) {
        ...
        const numero = this.obterNumeroOrcamento(dados) || projeto.numero || projeto.id || "-";
        ...
        estado.pagina.drawText("Or\u00e7amento", {
            x: direitaX,
            y: estado.y - 18,
            size: 9,
            font: estado.fonteNegrito,
        ...
        estado.pagina.drawLine({
            start: { x: estado.margem, y: estado.y - 110 },
            end: { x: estado.largura - estado.margem, y: estado.y - 110 },
            thickness: 1,
    ...
    desenharProjeto(estado, projeto = {}) {
        if (!this.projetoPreenchido(projeto)) {
            return;
        }
    ...
    projetoPreenchido(projeto = {}) {
        return !!(projeto?.nome || projeto?.numero || projeto?.id || projeto?.status || projeto?.endereco || projeto?.responsavel);
    },
    ...
    desenharProdutos(estado, produtos = [], totais = {}) {
        ...
        const colunas = [
            { titulo: "Item", x: estado.margem, largura: 24 },
            { titulo: "Descricao", x: estado.margem + 28, largura: 150 },
            { titulo: "Altura (cm)", x: estado.margem + 182, largura: 52 },
            { titulo: "Largura (cm)", x: estado.margem + 238, largura: 52 },
            { titulo: "Qtd", x: estado.margem + 294, largura: 30 },
            { titulo: "Valor (m2)", x: estado.margem + 328, largura: 70 },
            { titulo: "Subtotal", x: estado.margem + 402, largura: 112 }
        ];
        ...
        produtos.forEach(produto => {
            ...
            const tipoItem = this.rotuloItemPdf(produto.tipoItemNome || produto.nome);
            const descricaoInformada = this.rotuloItemPdf(produto.descricao);
            const descricao = [
                tipoItem,
                descricaoInformada && descricaoInformada !== tipoItem ? descricaoInformada : "",
                this.rotuloItemPdf(produto.subtipoItem)
            ].filter(Boolean).join(" - ");

            this.desenharLinhaTabela(estado, [
                produto.item,
                descricao || this.rotuloItemPdf(produto.nome) || "Item",
                this.formatarNumero(produto.alturaCm || produto.altura),
                this.formatarNumero(produto.larguraCm || produto.largura),
    ...
    desenharResumoFinanceiro(estado, dados = {}) {
        ...
        const totalFerragensAcessorios = Number(totais.totalAdicionais || 0) > 0
            ? Number(totais.totalAdicionais)
            : produtos.reduce(
                (total, produto) => total + Math.max(0, Number(produto.valorAdicional || 0)),
                0
            );
        const totalAluminio = Number(totais.totalAluminio || 0) > 0
            ? Number(totais.totalAluminio)
            : produtos.reduce((total, produto) => total + Math.max(0, Number(produto.totalAluminio || 0)), 0);
        const totalJato = Number(totais.totalJato || 0) > 0
            ? Number(totais.totalJato)
            : produtos.reduce((total, produto) => total + Math.max(0, Number(produto.valorJato || produto.adicionalJato || 0)), 0);
        const desconto = Math.max(0, Number(totais.desconto || 0));
        const totalComDesconto = Math.max(0, Number(totais.totalGeral || 0));
        const totalSemDesconto = totalComDesconto + desconto;

        this.desenharSecao(estado, "Resumo Financeiro");
        this.desenharCampos(estado, [
            ["Subtotal", this.formatarMoeda(totais.subtotal, moeda)],
            totalFerragensAcessorios > 0 ? ["Ferragens/Acess\u00f3rios", this.formatarMoeda(totalFerragensAcessorios, moeda)] : null,
            totalAluminio > 0 ? ["Aluminio", this.formatarMoeda(totalAluminio, moeda)] : null,
            totalJato > 0 ? ["Jato", this.formatarMoeda(totalJato, moeda)] : null,
            Number(totais.acrescimo || 0) > 0 ? ["Acrescimo", this.formatarMoeda(totais.acrescimo, moeda)] : null,
            [desconto > 0 ? "Total sem desconto" : "Total geral", this.formatarMoeda(totalSemDesconto, moeda)],
            desconto > 0 ? ["Desconto \u00e0 vista", this.formatarMoeda(desconto, moeda)] : null,
            desconto > 0 ? ["Valor \u00e0 vista", this.formatarMoeda(totalComDesconto, moeda)] : null
        ].filter(Boolean));
    },
    ...
    desenharLinhaTabela(estado, valores = [], colunas = [], cabecalho = false) {
        ...
        valores.forEach((valor, indice) => {
            ...
            const tamanhoFonte = cabecalho ? 9 : 8.7;
            const fonte = estado.fonteNegrito;
            const texto = this.limitarTextoParaLargura(fonte, this.limparTextoPdf(valor), tamanhoFonte, coluna.largura || 80);
    ...
    formatarMedidas(produto = {}) {
        ...
        return `${this.formatarNumero(altura)} x ${this.formatarNumero(largura)} cm`;
    },

    obterSubtotalProduto(produto = {}) {
        const total = this.primeiroNumero(produto, ["subtotalFinal", "valorTotal", "total", "totalGeral", "subtotal"], 0);
        return Math.max(0,
            total
            - Math.max(0, Number(produto.valorAdicional || 0))
            - Math.max(0, Number(produto.totalAluminio || 0))
            - Math.max(0, Number(produto.valorJato || produto.adicionalJato || 0))
        );
    },
