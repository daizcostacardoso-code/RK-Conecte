...
test("ordem de serviço carrega o contrato operacional na ordem segura", () => {
    ...
    assert.match(html, /nota-servico-model\.js\?v=1\.0\.0/);
    assert.match(html, /nota-servico-controller\.js\?v=1\.0\.0/);
});
...
test("dashboard inclui pendências de medição, produção e instalação", () => {
    ...
    assert.match(html, /dashboard-comercial-controller\.js\?v=1\.0\.0/);
});
